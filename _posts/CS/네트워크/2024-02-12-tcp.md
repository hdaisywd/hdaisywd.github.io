...
- Internet Layer에서는 IP 헤더가 생긴다. 시작 IP 주소가 입력되며 목적지 IP 주소의 정보 또한 입력된다.
👶 에? 근데 도메인 이름만 알지 않나? 
👉 Application Layer에서 DNS, Transport Layer에서 UDP를 통해 우리는 도메인 주소에서 IP 주소를 알아낼 수 있다. 
- Network Access Layer에서는 Ethernet 프로토콜을 사용하는데, 이를 위해 MAC 주소가 있어야 한다. 여기서 말하는 MAC 주소는 우리집 공유기의 MAC 주소이다. 
